...
When the post-rotation is supplied externally (``model-free'' case) we instead work with
\begin{equation}
  \eta_{\mathrm{mix}} \coloneqq
  \max\left\{ \norm{\Pi_{\mathrm{null}} P_{\mathrm{sp}} \Pi_{\mathrm{act}}},\;
              \norm{\Pi_{\mathrm{act}} P_{\mathrm{sp}} \Pi_{\mathrm{null}}} \right\}.
\end{equation}
...
\begin{lemma}[Model-free bound]\label{lem:model-free}
For an arbitrary $P_{\mathrm{sp}} \in SO(d_h)$ written in the STRING basis as
$U \begin{bmatrix} A & B \\ C & D \end{bmatrix} U^\top$ we have
\begin{equation}
  \norm{ \Pi_{\mathrm{act}} P_{\mathrm{sp}} \Pi_{\mathrm{act}} - \Pi_{\mathrm{act}} }
  \le 2\,\eta_{\mathrm{mix}}.
\end{equation}
\end{lemma}

Combining Lemmas~\ref{lem:quadratic-mix} and \ref{lem:model-free} yields a unified parameter
\begin{equation}
  \delta_{\mathrm{mix}} \coloneqq
  \begin{cases}
    \dfrac{2}{(1-\rho)^3}\,\eta^2, & \text{structured Cayley / ESPR case}, \\[6pt]
    2\,\eta_{\mathrm{mix}}, & \text{model-free case}.
  \end{cases}
\end{equation}
...
Let $C_{\mathrm{mix}} = 1$ and $\delta_{\mathrm{mix}} = \eta_{\mathrm{mix}}$ in the model-free case, and
$C_{\mathrm{mix}} = \frac{2}{(1-\rho)^3}$ together with $\delta_{\mathrm{mix}} = \eta^2$ in the structured Cayley case.
Then
\begin{align}
  \abs{\alpha_{ij} - \alpha_{ij}^\star}
  &\le
...
\begin{itemize}
  ...
  \item Lemmas~\ref{lem:cayley-lip}--\ref{lem:model-free}, Corollary~\ref{cor:active-block}, Lemma~\ref{lem:logit-pert},
